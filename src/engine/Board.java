...
public class Board extends BoardAbstract {
	...
	// over loaded board constructor to accept SidePanel (editor) if editor is
	// to be enabled
	...
	private void initBoard() {
		...
		// this.diagnosticsOverlay = renderingEngine.addOverlay( new
		// DiagnosticsOverlay() );
		 ...
		 * ####################################
		 * 
		 * In progress of factorization. For now each collidable and sprite are
		 * manually added to the Collision and Rendering Engines, respectively.
		 * 
		 * Eventually the factory will take care of this upon construction of
		 * the composites, and/or the decorators will take the rendering and
		 * collision engines as arguments.
		 * 
		 * ####################################
		 */
		...
		Line2D[] triangleBounds = new Line2D[] { new Line2D.Double(-25, -50, 2000, 500),
				new Line2D.Double(2000, 500, -25, 500), new Line2D.Double(-25, 500, -25, -50) };
		...
		// laser = new Tracer(143,260, physicsEntitiesList.get(0) , this );
		// //later will be parent system
		// dynamicEntitiesList.add(new LaserTest(400,60)); <-- can't add as long
		// as I don't have a sprite for it
		...
		renderingEngine.layersList[7]
				.addGraphicToLayer(EntityFactory.createBackgroundSprite("Prototypes/Sky.png", 0, 0));

		camera.setTarget(player);

		currentScene.addBackgroundSprite(6,
				EntityFactory.createBackgroundSprite("Prototypes/L7.png", 400 - offset_x, 150 - offset_y));
		currentScene.addBackgroundSprite(5,
				EntityFactory.createBackgroundSprite("Prototypes/L6.png", 260 - offset_x, -450 - offset_y));// bass
		currentScene.addBackgroundSprite(4,
				EntityFactory.createBackgroundSprite("Prototypes/L5_02.png", 650 - offset_x, 400 - offset_y));
		currentScene.addBackgroundSprite(3,
				EntityFactory.createBackgroundSprite("Prototypes/L4.png", 300 - offset_x, -300 - offset_y));// base
		currentScene.addBackgroundSprite(2,
				EntityFactory.createBackgroundSprite("Prototypes/L5_03.png", 650 - offset_x, 500 - offset_y)); // forest
		currentScene.addBackgroundSprite(1,
				EntityFactory.createBackgroundSprite("Prototypes/L2.png", 250 - offset_x, -160 - offset_y));// pipe
		currentScene.addBackgroundSprite(0,
				EntityFactory.createBackgroundSprite("Prototypes/L1_01.png", 300 - offset_x, -160 - offset_y));// base

		// renderingEngine.layersList[4].addEntity(
		// EntityFactory.createBackgroundScroll( "Prototypes/shader_rain01.png",
		// getBoardWidth() ,getBoardHeight() ,0,-3 ) );
		renderingEngine.layersList[1].addGraphicToLayer(EntityFactory
				.createBackgroundScroll("Prototypes/shader_rain03.png", getBoardWidth(), getBoardHeight(), 0, -8));

		initBullets();

		/*
		 * //ADD COLLIDABLES TO COLLISION ENGINE\ //TO BE MOVED TO MASTER ADDING
		 * FACTOR THAT SORTS COMPOSITES AND PASSES THEM TO RESPECTIVE ENGINES
		 * 
		 * for ( EntityStatic stat : this.currentScene. ){
		 * collisionEngine.addStaticCollidable( (Collider)
		 * stat.getCollisionType() ); }
		 */
		initEditorPanel();
	...
	/*
	 * private void updatePhysicsEntities() { //for (EntityDynamic dynamicEntity
	 * : dynamicObjects) { for (int i = 0 ; i < physicsEntitiesList.size() ;
	 * i++){ EntityDynamic physicsEntity = physicsEntitiesList.get(i);
	...
	public void initBullets() {
		...
		/*
		 * for (int[] p : pos) { dynamicEntitiesList.add(new Bullet(p[0],
		 * p[1],-1,1)); }
		 */
	...
	protected class MouseHandlerClass extends MouseInputAdapter {
		/*
		 * public int clickPositionXOffset; public int clickPositionYOffset;
		 */
	...
	/*
	 * @Override public MovingCamera getCamera() { return this.camera; }
	 */
	...
	// }
	public ArrayList<EntityDynamic> getDynamicEntities() {
